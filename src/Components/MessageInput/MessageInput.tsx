    ...
    useEffect(() => {
        const listener = (event: KeyboardEvent) => {
            ...
            if (event.code === 'Enter' && !event.shiftKey && input.current) {
                ...
                const textSegments = getTextSegments(input.current);
                const textContent = textSegments.map(({text}) => text).join('');
                console.info(`Sent new message ${textContent}`)
    ...
    return (
        <div className={'message-input'}>
            ...
            <div className={'message-input--emojiToggle'}>
                <svg className={'message-input--emojiToggle---button'} onClick={togglePicker} width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
